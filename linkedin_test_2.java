import java.util.*;
/* question 
find the weight of the given list with depth in reverse order
eg :- {{1,1},2,{1,1}} = 10
		
		 2   			level = 2     2*2 = 4
		/ \
	   /   \
	(1,1)  (1,1)		level = 1      1 * ((1+1) + (1+1)) = 4
  ...
  */
  ...
  class depthReverse {
	...
	public static void main(String[] args) {
        ...
        // List<Object> in6 = new ArrayList<>();
        // in6.add(new Integer(1));
        // in6.add(new Integer(1));
    ...
    public static void findDepth(List<Object> input) {
        ...
        int depth = findDepth(input,1,false);
        System.out.println("depth :- " + depth);
        System.out.println("Final sum :- " + calculateDepth(input,depth,0));
    }

    public static int findDepth(List<Object> input,int level,boolean insideList) {
        for(Object i : input) {
            if(isList(i)) {
                if(!insideList) {
                    insideList = true;
                ...
                level = findDepth(getList(i),level,insideList);
            } else {
                insideList = false;
            }
        }
